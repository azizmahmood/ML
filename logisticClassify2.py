    ...
    def train(self, X, Y, initStep=1.0, stopTol=1e-4, stopEpochs=5000, plot=None):
        ...
        while not done:
            ...
            for i in np.random.permutation(M):
                ...
                gradi = -YY[i]*(1 - self.sig(ri))*(XX[i]) - (1 - YY[i])*(-self.sig(ri))*XX[i];    # TODO: compute gradient of NLL loss
            ...
            Jsur = 0;
            for i in np.random.permutation(M):
                Jsur += -YY[i]*np.log(self.sig(np.dot(self.theta, XX[i]))) -(1-YY[i])*np.log(1-self.sig(np.dot(self.theta, XX[i])));

            Jsur = Jsur/M;
            #Jnll.append( NotImplementedError ) # TODO evaluate the current NLL loss
            if stopEpochs <= epoch or (len(Jnll) >= 2 and np.abs(Jnll[-1] -Jnll[-2]) < stopTol):
                done = True;
                plt.figure(1); plt.plot(Jnll,'b-',J01,'r-'); plt.draw();
                # plot losses
                if N==2: plt.figure(2); self.plotBoundary(X,Y); # & predictor if 2D
